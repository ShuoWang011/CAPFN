...
class OneModel(nn.Module):
    def __init__(self, args):
        ...
        #self.criterion = nn.NLLLoss(ignore_index=args.ignore_label)
    ...
    def forward(self, x, y):
        ...
        x = self.ppm(x)
        x = self.cls(x)
        #print('cls后interpolate前x:',x.shape) b,61,27,27

        if self.zoom_factor != 1:
            x = F.interpolate(x, size=(h, w), mode='bilinear', align_corners=True)

        if self.training:
            #print('interpolate后criterion前x:',x.shape) b,61,417,417
            #print('criterion前y:',y.long().shape) b,417,417
            #print('criterion前y值:', y.data)
            main_loss = self.criterion(x, y.long())
            #print('criterion后x:',x.max(1)[1].shape) b,417,417
            #print('mainloss',main_loss.shape) []
