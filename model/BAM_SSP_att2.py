...
def SA_Weighted_GAP(supp_feat, mask, supp_pred_mask):
    ...
    new_mask3 = torch.zeros_like(mask) #!!!!!!!!
    new_mask4 = torch.zeros_like(mask) #!!!!!!!!
    ...
    new_mask3[supp_pred == 1] = 1 #!!!!!!!! delta
    new_mask3[mask == 1] = 0 #!!!!!!!!

    new_mask4[supp_pred == 0] = 1 #!!!!!!!! gamma
    ...
    new_area3 = F.avg_pool2d(new_mask3, (supp_feat.size()[2], supp_feat.size()[3])) * feat_h * feat_w + 0.0005 #!!!!!!!!
    new_area4 = F.avg_pool2d(new_mask4, (supp_feat.size()[2], supp_feat.size()[3])) * feat_h * feat_w + 0.0005 #!!!!!!!!
    ...
    supp_feat3 = supp_feat * new_mask3 #!!!!!!!!
    supp_feat3 = F.avg_pool2d(input=supp_feat3, kernel_size=supp_feat.shape[-2:]) * feat_h * feat_w / new_area3 #!!!!!!!!
    supp_feat4 = supp_feat * new_mask4 #!!!!!!!!
    supp_feat4 = F.avg_pool2d(input=supp_feat4, kernel_size=supp_feat.shape[-2:]) * feat_h * feat_w / new_area4 #!!!!!!!!
    return supp_feat1, supp_feat2,supp_feat3,supp_feat4
...
def get_gram_matrix(fea):
    ...
    return gram
def similarity_func(feature_q, fg_proto, bg_proto):
...
def SSP_func(feature_q, out):
        ...
        for epi in range(bs):
            ...
            bg_thres = 0.6 #0.6
            cur_feat = feature_q[epi].view(256, -1)
            ...
            fg_proto_local = fg_proto_local.t().view(256, f_h, f_w).unsqueeze(0) # 1024, N3
            bg_proto_local = bg_proto_local.t().view(256, f_h, f_w).unsqueeze(0) # 1024, N3
...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        self.cls_type = cls_type  # '1Base' or 'Novel'
        self.layers = args.layers  # 50
        self.zoom_factor = args.zoom_factor  # 8
        self.shot = args.shot  # 1
        self.vgg = args.vgg
        self.dataset = args.data_set
        self.criterion = nn.CrossEntropyLoss(ignore_index=args.ignore_label)

        self.print_freq = args.print_freq / 2  # 5
        ...
        weight_path = './initmodel/PSPNet/{}/split{}/{}/best.pth'.format(
            args.data_set, args.split,
            backbone_str)  # C:\Users\zyr\Desktop\Natural_image\Projects\BAM-main\BAM\exp\pascal\PSPNet\split0\vgg\snapshot\best.pth
    ...
    def forward(self, x, s_x, s_y, y_m, y_b, cat_idx=None):
        ...
        for i in range(self.shot):
            mask = (s_y[:, i, :, :] == 1).float().unsqueeze(1)#b,1,473,473
        ...
        supp_pro_bg = torch.cat(supp_feat_BG_list, 2)  # [bs, 256, shot, 1]#!!!!!!!!
        supp_pro_bg = (weight_soft.permute(0, 2, 1, 3) * supp_pro_bg).sum(2, True)#!!!!!!!!
#====================================================================
        out_0 = similarity_func(query_feat, supp_pro, supp_pro_bg)#b,2,30,30
        out_0_new = out_0.softmax(1)
        out_0_new = out_0_new[:, 1:, :, :]
        out_0_new = F.interpolate(out_0_new, size=(h, w), mode='bilinear', align_corners=True)


        mask_q = (y_m[:, :, :] == 1).float().unsqueeze(1)  # b,473,473
        #print('out_0:', out_0.shape)
        #print(out_0)
        #gam = self.gam(query_feat, mask_q)#-------------------
        gam = self.gam(query_feat, out_0_new)  # -------------------

        ##################### Self-Support Prototype (SSP) #####################
        SSFP_1, SSBP_1, ASFP_1, ASBP_1 = SSP_func(query_feat, out_0)

        FP_1 = supp_pro * 0.5 + SSFP_1 * 0.5
        #BP_1 = SSBP_1 * 0.3 + ASBP_1 * 0.7

        # Tile & Cat
        concat_feat = FP_1.expand_as(query_feat)
        #merge_feat = torch.cat([query_feat, concat_feat, corr_query_mask], 1)  # 256+256+1
        merge_feat = torch.cat([query_feat, concat_feat, corr_query_mask, gam], 1)  # 256+256+1+256 ----------
        #print("merge_feat:",merge_feat.shape)
        merge_feat = self.init_merge(merge_feat)
        #out_1 = similarity_func(query_feat, FP_1, BP_1)
