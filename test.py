...
from model import BAM_SCL_DCPPP_reloss3_nbs12
...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='BAM_SCL_DCPPP_reloss3_nbs12')
...
def validate(val_loader, model, val_seed):
    ...
    db_epoch = math.ceil(test_num/(len(val_loader)-args.batch_size_val)) #1
    iter_num = 0
    ...
    for e in range(db_epoch):
        ...
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b) in enumerate(val_loader):
            ...
            if i ==319:
                torchvision.utils.save_image(input, os.path.join(args.output_path, 'i_plane{}.png'.format(num)))
                torchvision.utils.save_image(target.float(), os.path.join(args.output_path, 't_plane{}.png'.format(num)))
                torchvision.utils.save_image(output.float(), os.path.join(args.output_path, 'o_plane{}.png'.format(num)))
            if i ==176:
                torchvision.utils.save_image(input, os.path.join(args.output_path, 'i_bicycle{}.png'.format(num)))
                torchvision.utils.save_image(target.float(), os.path.join(args.output_path, 't_bicycle{}.png'.format(num)))
                torchvision.utils.save_image(output.float(), os.path.join(args.output_path, 'o_bicycle{}.png'.format(num)))
            if i ==241:
                torchvision.utils.save_image(input, os.path.join(args.output_path, 'i_bird{}.png'.format(num)))
                torchvision.utils.save_image(target.float(), os.path.join(args.output_path, 't_bird{}.png'.format(num)))
                torchvision.utils.save_image(output.float(), os.path.join(args.output_path, 'o_bird{}.png'.format(num)))
            if i ==228:
                torchvision.utils.save_image(input, os.path.join(args.output_path, 'i_boat{}.png'.format(num)))
                torchvision.utils.save_image(target.float(), os.path.join(args.output_path, 't_boat{}.png'.format(num)))
                torchvision.utils.save_image(output.float(), os.path.join(args.output_path, 'o_boat{}.png'.format(num)))
            if i ==74:
                torchvision.utils.save_image(input, os.path.join(args.output_path, 'i_bottle{}.png'.format(num)))
                torchvision.utils.save_image(target.float(), os.path.join(args.output_path, 't_bottle{}.png'.format(num)))
                torchvision.utils.save_image(output.float(), os.path.join(args.output_path, 'o_bottle{}.png'.format(num)))
