...
def get_parser():
    ...
    parser.add_argument('--arch', type=str, default='BAM_SSP_att') #
    parser.add_argument('--viz', action='store_true', default=False)
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0_vgg.yaml', help='config file') # coco/coco_split0_resnet50.yamlpascal/pascal_split0_vgg.yaml
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        '''
        print('input:',input.shape) # b,3,473,473
        print('target:', target.shape) #b,473,473
        print('target_b:', target_b.shape) #b,473,473
        print('s_input:', s_input.shape) #b,1,3,473,473
        print('s_mask:', s_mask.shape) #b,1,473,473
        print('subcls:', subcls)
        plt.imshow(np.asarray(input.resize(3,473,473)).transpose(1,2,0))
        plt.imshow(np.asarray(target.resize(473, 473)))
        plt.imshow(np.asarray(target_b.resize(473, 473)))
        plt.imshow(np.asarray(s_input.resize(3, 473, 473)).transpose(1, 2, 0))
        plt.imshow(np.asarray(s_mask.resize(473, 473)))
        plt.show()
        '''
