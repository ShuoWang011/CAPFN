...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/pascal/pascal_split0_vgg_base.yaml', help='config file') # pascal/pascal_split0_vgg_base.yamlcoco/coco_split0_resnet50.yamlcoco/coco_split0_vgg_base.yaml
...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        '''
        plt.imshow(input.reshape(3,417,417).permute(1,2,0))  # image表示待处理的图像
        plt.show()
        plt.imshow(target.reshape(417,417))
        plt.show()
        '''
        ...
        #print('input:',input.shape) b,3,417,417
        #print('input[]值:', input[0,:,115,281])
        #print('target:',target.shape) b,417,417
        #print('target[]值:', target[0, 115, 281])
        
        output, main_loss = model(x=input, y=target)
        #print('output:',output.shape) b,417,417
        #print(main_loss.shape) []
